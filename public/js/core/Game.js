...
class Game {
    ...
    init() {
        ...
        // Event listeners for celebration effects
    ...
    _onLevelUp(level) {
        ...
        if (this.notificationUI) {
            this.notificationUI.show(
                `Level ${level}!`,
                i18n.lang === 'nl' ? 'Je bent sterker geworden!' : 'You leveled up!',
                'levelup'
            );
        }
    ...
    _onBadgeEarned(badgeId) {
        ...
        if (this.notificationUI) {
            const name = i18n.lang === 'nl' ? badge.name_nl : badge.name_en;
            this.notificationUI.show(
                `${badge.icon} ${name}`,
                i18n.lang === 'nl' ? 'Badge verdiend!' : 'Badge earned!',
                'badge'
            );
        }
