...
class ChunkManager {
    ...
    createTree(height) {
        ...
        const trunkMat = new THREE.MeshLambertMaterial({ color: treeCfg.trunkColor || 0x4a3728 });
        const trunk = new THREE.Mesh(trunkGeom, trunkMat);
        trunk.position.y = height * 0.2;
        tree.add(trunk);

        const foliageMat = new THREE.MeshLambertMaterial({ color: treeCfg.foliageColor || 0x1a4a1a });
    ...
    createCrystal(height) {
        ...
        for (let i = 0; i < count; i++) {
            const shard = new THREE.Mesh(crystalGeom, crystalMat);
            shard.position.set(
                (Math.random() - 0.5) * 0.5,
                height * (0.3 + i * 0.25),
                (Math.random() - 0.5) * 0.5
