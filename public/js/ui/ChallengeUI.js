...
class ChallengeUI {
    ...
    _showFeedback(result) {
        ...
        feedbackEl.innerHTML = `
            ...
            <div class="feedback-label">${scoreLabel}</div>
            <div class="feedback-text">${result.feedback}</div>
            <div class="feedback-xp">+${result.xp} XP</div>
            ${result.nextRound ? `<p class="feedback-round">${i18n.t('boss.round')} ${result.round}/${result.totalRounds}</p>` : ''}
        `;
