...
class MentorSystem {
    ...
    _addMentorIndicator(group, height) {
        ...
        orb.userData.orbBaseY = height;
        group.add(orb);
        // Cache direct reference to avoid traverse() per frame
    ...
    update(delta, playerPosition) {
        ...
        for (const mentor of this.mentors) {
            ...
            // Idle animation - bob mentor orb using cached reference
            const orb = mentor.userData.orbRef;
            if (orb) {
                orb.position.y = orb.userData.orbBaseY + Math.sin(Date.now() * 0.003 + mentor.position.x) * 0.3;
